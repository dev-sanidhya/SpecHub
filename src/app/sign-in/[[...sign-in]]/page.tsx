...
export default function SignInPage() {
  ...
  const handleSocial = async (provider: "oauth_github" | "oauth_google") => {
    ...
    try {
      ...
      const clerkErr = err as { errors?: { message: string }[] };
      setError(clerkErr.errors?.[0]?.message ?? "OAuth sign in failed. Try again.");
    ...
    <div className="min-h-screen flex items-center justify-center bg-background">
      ...
      <div className="w-full max-w-sm px-4">
          ...
          {!isLoaded ? (
            ...
            <>
              <div className="mb-6 text-center">
                <h1 className="text-xl font-semibold text-foreground">Sign in to SpecHub</h1>
                <p className="text-sm text-foreground-2 mt-1">Welcome back! Please sign in to continue</p>
              </div>
        ...
        <p className="text-center text-sm text-foreground-3 mt-6">
          ...
          <a href="/sign-up" className="text-indigo-500 hover:text-indigo-400 transition-colors">Sign up</a>
        </p>
      </div>
    </div>
  );
